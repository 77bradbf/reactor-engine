...
It is a Python command-line tool for common chemical reaction engineering calculations. The code can compute reactor design, conversion profiles, temperature profiles, reaction selectivity, reaction yield, kinetic parameters from experimental data, parallel-reaction CSTR optimization, pure-species property lookups, and non-isothermal PFR calculations using real heat capacities from property libraries.
...
\textbf{Copy-paste note.} Long commands are written with backslashes. Copy the whole command block at once. Each line that ends in \texttt{\textbackslash} tells Terminal that the command continues on the next line. Do not copy a line that has only \texttt{--}; every option must stay together, such as \texttt{--parallel-ku2}.
...
\subsection{Advanced Libraries Used by the Project}

The advanced requirements file includes several scientific libraries. The current code uses some directly and keeps others available for future extensions:

\begin{center}
\begin{tabular}{p{0.25\linewidth} p{0.62\linewidth}}
\toprule
\textbf{Library} & \textbf{How It Helps} \\
\midrule
\texttt{pandas} & Reads CSV and Excel tables for kinetic fitting when available. \\
\texttt{openpyxl} & Lets \texttt{pandas} read \texttt{.xlsx} Excel files. \\
\texttt{thermo} & Provides chemical properties and molar heat capacities for real species. \\
\texttt{CoolProp} & Provides thermophysical properties for many common fluids. \\
\texttt{pint} & Installed for future unit-aware calculations. \\
\texttt{lmfit} & Installed for future nonlinear kinetic fitting and uncertainty estimates. \\
\texttt{cantera} & Installed for future detailed kinetics, thermodynamics, and reactor networks. \\
\bottomrule
\end{tabular}
\end{center}
...
\texttt{species-properties} & Looks up real pure-species properties with \texttt{thermo} or \texttt{CoolProp}. \\
\texttt{species-nonisothermal-pfr} & Solves a non-isothermal PFR using real species heat capacities from \texttt{thermo} or \texttt{CoolProp}. \\
\bottomrule
\end{tabular}
\end{center}
...
python3 reactor_engine.py \
  ...
  --fit-data-csv examples/power_fit_example.xlsx \
  --no-show
\end{lstlisting}
...
python3 reactor_engine.py \
  --mode kinetic-fit-power-law \
  --fit-data-csv examples/kinetic_fit_photo.csv \
  --catalyst-weight 40 \
  --no-show
\end{lstlisting}
...
\section{Mode 9: Species Properties with \texttt{thermo} or \texttt{CoolProp}}

\subsection{What the Code Computes}

This mode looks up real pure-species properties at a specified temperature and pressure. It can use \texttt{thermo}, \texttt{CoolProp}, or \texttt{auto}. With \texttt{auto}, the code tries \texttt{thermo} first and then \texttt{CoolProp}.

Depending on the backend and species, the output can include molecular weight, phase, density, heat capacity, enthalpy, entropy, vapor pressure, viscosity, thermal conductivity, critical properties, and other available properties.

\subsection{Minimum Inputs}

\begin{itemize}
    \item \texttt{--mode species-properties}
    \item \texttt{--species}: chemical name or backend-recognized species name
    \item \texttt{--temperature}: temperature in K
    \item \texttt{--pressure}: pressure in Pa
    \item \texttt{--property-backend}: \texttt{auto}, \texttt{thermo}, or \texttt{coolprop}
\end{itemize}

\subsection{Example 9A: Water Properties with Automatic Backend}

\textbf{Problem statement.} Find real pure-water properties at 350 K and 1 atm. Let the code choose the backend automatically.
...
python3 reactor_engine.py \
  ...
  --temperature 350 \
  --pressure 101325 \
  --property-backend auto \
  --no-show
\end{lstlisting}

\subsection{Example 9B: Ethanol Properties Forced Through \texttt{thermo}}

\textbf{Problem statement.} Find ethanol properties at 330 K and 1 atm using the \texttt{thermo} library.
...
python3 reactor_engine.py \
  ...
  --temperature 330 \
  --pressure 101325 \
  --property-backend thermo \
  --no-show
\end{lstlisting}

\subsection{Example 9C: Water Properties Forced Through \texttt{CoolProp}}

\textbf{Problem statement.} Find water properties at 350 K and 1 atm using \texttt{CoolProp}. Some \texttt{CoolProp} names use capital letters, such as \texttt{Water}.
...
python3 reactor_engine.py \
  ...
  --temperature 350 \
  --pressure 101325 \
  --property-backend coolprop \
  --no-show
\end{lstlisting}

\section{Mode 10: Species-Based Non-Isothermal PFR}

\subsection{What the Code Computes}

This mode solves the same coupled PFR design and energy balance as \texttt{nonisothermal-pfr}, but it calculates the heat-capacity flow using real species molar heat capacities from \texttt{thermo} or \texttt{CoolProp}:

\begin{align}
\frac{dX}{dV} &= \frac{-r_A}{F_{A0}} \\
\frac{dT}{dV} &= \frac{Ua(T_a - T) + (-r_A)(-\Delta H_{rxn})}{\sum F_i C_{p,i}(T,P)}
\end{align}

This is more realistic than entering constant \texttt{--cp-a}, \texttt{--cp-b}, and \texttt{--cp-p} by hand.

\subsection{Minimum Inputs}

\begin{itemize}
    \item reactor/feed inputs: \texttt{--volume}, \texttt{--fa0}, \texttt{--fb0}, \texttt{--v0}, \texttt{--t0}
    \item species names: \texttt{--species-a}, \texttt{--species-b}, \texttt{--species-p}
    \item heat inputs: \texttt{--delta-h}, \texttt{--ua}, \texttt{--ta}
    \item property inputs: \texttt{--pressure}, \texttt{--property-backend}
    \item optional inert: \texttt{--fi0} and \texttt{--species-i}
\end{itemize}

\subsection{Example 10A: Ethanol Oxidation-Type PFR with Real Heat Capacities}

\textbf{Problem statement.} A PFR is modeled with A = ethanol, B = oxygen, and P = acetaldehyde. The feed has $F_{A0}=2$ mol/time and $F_{B0}=3$ mol/time, with $v_0=50$ volume/time and $T_0=330$ K. The reactor volume is 50, $\Delta H_{rxn}=-50000$ J/mol A, $Ua=120$, and $T_a=310$ K. Compute conversion and exit temperature using real heat capacities from \texttt{thermo}.
...
\subsection{Example 10B: Same PFR with an Inert Diluent}

\textbf{Problem statement.} Repeat the previous reactor, but add 5 mol/time of nitrogen inert. Because an inert is present, include both \texttt{--fi0} and \texttt{--species-i}.
...
\subsection{Example 10C: Show the Conversion and Temperature Plots}

Remove \texttt{--no-show} to open the Matplotlib plots:
...
\subsection{For Real Species Property Problems}

Ask:

\begin{enumerate}
    \item Which species names should the property library use?
    \item What are the temperature and pressure?
    \item Should the backend be \texttt{auto}, \texttt{thermo}, or \texttt{coolprop}?
    \item For species-based PFRs, which species correspond to A, B, product P, and optional inert I?
    \item Is the heat of reaction exothermic or endothermic, and are the energy units consistent?
\end{enumerate}
...
\texttt{--species} & Pure species name for property lookup. \\
\texttt{--species-a}, \texttt{--species-b}, \texttt{--species-p} & Species names for reactant A, reactant B, and product P. \\
\texttt{--species-i} & Optional inert species name when \texttt{--fi0} is nonzero. \\
\texttt{--pressure} & Pressure for real-property calculations, Pa. \\
\texttt{--property-backend} & Selects \texttt{auto}, \texttt{thermo}, or \texttt{coolprop}. \\
...
\begin{itemize}
    ...
    \item Use \texttt{species-properties} when the problem asks for real physical properties of a chemical species.
    \item Use \texttt{species-nonisothermal-pfr} when the energy balance should use real species heat capacities from property libraries.
\end{itemize}
